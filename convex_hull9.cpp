...
double convex_hull_area(std::vector<Point *> &points) {
	...
	// Ustawiam najniższy punkt na początek i przypisuję do globalnej zmiennej
	std::swap(points[0], points[l_i]);
	// Sortuję punkty według kąta nachylenia względem punktu startowego
	std::sort(points.begin() + 1, points.end(), cmp_orientation);
	// Usuwam współliniowe punkty
	remove_collinear(points);
	// Znajduję otoczkę wypukłą
	std::stack<Point *> s;
