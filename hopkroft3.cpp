		...
		switch (v.type) {
			case 1:
				...
				if (x + 3 < size && board[y][x + 3]) {
					white_idx = size * y + x + 3;
					white_type = board[y][x + 3];
					add_captured(white_idx, white_type, v);
				}
				//* 2 do przodu + 1 lewo/prawo
				if (x + 2 < size) {
					if (y - 1 >= 0 && board[y - 1][x + 2]) {
						white_idx = v.idx - size + 2;
						white_type = board[y - 1][x + 2];
						add_captured(white_idx, white_type, v);
					}
			...
			case 2:
				...
				if (x + 1 < size && board[y][x + 1]) {
					white_idx = size * y + x + 1;
					white_type = board[y][x + 1];
					add_captured(white_idx, white_type, v);
				}
				//* 1 lewo
				if (y - 1 >= 0 && board[y - 1][x]) {
					white_idx = x - size;
					white_type = board[y - 1][x];
					add_captured(white_idx, white_type, v);
				}
				//* 1 prawo
		...
		if (h < max_h) {
			for (int e: bip_graph[i].edges) {
				...
				if (dist[e_idx] == dist[i] + 1) {
					if (dfs(e_idx, h + 1, max_h, !is_black)) {
						if (is_black) {
							bip_graph[i].edge = e;
							bip_graph[e_idx].edge = i;
						}
						return true;
					}
				}
			}
		} else if (h == max_h && bip_graph[i].edge == -1 && !is_black) {
			return true;
		}
		return false;
	}
